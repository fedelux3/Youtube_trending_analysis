...
def query1(coll):
    '''
    Inizializza i campi "covid_tags" e "covid_title" a false su tutti i documenti
    '''
    query = {}
...
def query2(coll, ER):
    '''
    Applica l'espressione regolare a tutti i tag dei documenti e in caso di riscontro
    fissa il campo "covid_tags" a true.
    '''
...
def query3(coll, ER):
    '''
    Applica l'espressione regolare a tutti i title dei documenti e in caso di riscontro
    fissa il campo "covid_title" a true.
    '''
    regex = re.compile(ER, re.IGNORECASE)
